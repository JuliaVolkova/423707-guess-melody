...
import {resultSuccessScreen} from './resultSuccess';
import {failTimeScreen} from './failTime';
import {failTriesScreen} from './failTries';
import {modalConfirmScreen} from './modalConfirm';
...
export const gameArtistScreen = createElementFromHtml(gameArtistTemplate);
...
const possibleScreens = [resultSuccessScreen, failTimeScreen, failTriesScreen];
const chooseRandomIndex = (min, max) => Math.floor(Math.random() * (max - min) + min);
const chooseRandomElement = (array) => array[chooseRandomIndex(0, array.length)];

const answerList = Array.from(gameArtistScreen.querySelectorAll(`.artist`));
for (let answer of answerList) {
  answer.addEventListener(`click`, () => renderScreen(chooseRandomElement(possibleScreens)));
}
